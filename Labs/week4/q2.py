...
def jacobi_method(a: np.ndarray, b: np.ndarray, error_targ: float = 10**-12):
    def _jacobi_method(x_vec: np.ndarray, n: int, abs_errors: float):
        ...
        return "Does not converge" if (len(abs_errors) > 30 and abs_error > abs_errors[-20]) or n >= 500 else x_vec if abs_error < error_targ else _jacobi_method(new_x_vec, n+1, abs_errors + [abs_error])
...
def gs_method(a: np.ndarray, b: np.ndarray, error_targ: float = 10**-12):
    def _gs_method(x_vec: np.ndarray, n: int, abs_errors: float):
        ...
        return "Does not converge" if (len(abs_errors) > 30 and abs_error > abs_errors[-20]) or n >= 500 else x_vec if abs_error < error_targ else _gs_method(new_x_vec, n+1, abs_errors + [abs_error])
